...
global DATA_PATH 
DATA_PATH = r'../Data/GPT2SP Data/Split/'
ROW_MAE, ROW_MMRE, ROW_PRED = 3,4,5

# TRAIN_TEST_FILE_PAIRS=os.listdir(DATA_PATH)
# TRAIN_TEST_FILE_PAIRS.sort(key = str.lower)
TRAIN_TEST_FILE_PAIRS = ["appceleratorstudio", "aptanastudio", "bamboo", "clover", "datamanagement", "duracloud", "jirasoftware",
...
def data_processing(file_pair):
    ...
    if WITHIN_PROJECT:
        ...
        train_data = pd.read_csv(DATA_PATH+str(file_pair)+"_train.csv")
        train_text = train_data["Issue"].tolist()
        train_labels = train_data["Storypoint"].tolist()

        val_data = pd.read_csv(DATA_PATH + str(file_pair) + "_val.csv")
        val_text = val_data["Issue"].tolist()
        val_labels = val_data["Storypoint"].tolist()

        test_data = pd.read_csv(DATA_PATH + str(file_pair) + "_test.csv")
        test_text = test_data["Issue"].tolist()
        test_labels = test_data["Storypoint"].tolist()
    # define batch size dynamicalloutputsy based on training length
    if DYNAMIC_BATCH:
        BATCH_SIZE = int(len(train_text) * BATCH_SIZE_RATIO)
    # tokenization
    # ttl = train_text.tolist()
    # for i in range(len(ttl)):
    #     ttl[i] = str(ttl[i])
    # vtl = val_text.tolist()
    # for i in range(len(vtl)):
    #     vtl[i] = str(vtl[i])
    tokens_train = tokenization(train_text)
    tokens_val = tokenization(val_text)
 
    train_seq = torch.tensor(tokens_train['input_ids'])
    # print(train_ex)
    # train_ex = np.array(train_ex)
    # train_ex = torch.tensor(train_ex)
    train_y = torch.tensor(train_labels).type(torch.FloatTensor)
    ...
    val_y = torch.tensor(val_labels).type(torch.FloatTensor)
    ...
    test_file_names = []
    if WITHIN_PROJECT:
        # ttl = test_text.tolist()
        # for i in range(len(ttl)):
        #     ttl[i] = str(ttl[i])
        tokens_test = tokenization(test_text)
        test_seq = torch.tensor(tokens_test['input_ids'])
        # test_ex = np.array(test_ex)
        # test_ex = torch.tensor(test_ex)
        # test_seq = torch.cat((test_ex,test_seq),dim=1)
        test_y = torch.tensor(test_labels).type(torch.FloatTensor)
...
def within_project_split(data):
    ...
    train_labels = (data['storypoint'][:train_val_split_point])
    ...
    val_labels = (data['storypoint'][train_val_split_point:val_test_split_point])
    test_ex=data.iloc[val_test_split_point:,0:3]
    test_text = data['text'][val_test_split_point:]
    test_labels = (data['storypoint'][val_test_split_point:])
...
def train_eval_test(file_pair, train_dataloader, val_dataloader, all_test_dataloader, model, test_file_names):
        ...
        for test_dataloader in all_test_dataloader:
            ...
            for i in range(len(predictions)):
                for j in range(len(predictions[i])):
                    ...
                    preds_for_sp.append(predictions[i][j])
                    trues_for_sp.append(true_labels[i][j])
            ...
            sp = scipy.stats.spearmanr(trues_for_sp, preds_for_sp).statistic
            spearmans_records.append(sp)
    ...
    print(file_pair)
    print(str(MAE_RECORDS[min_eval_loss_epoch[1]])  + '\n'+str(MMRE_RECORDS[min_eval_loss_epoch[1]]) + '\n'+ str(PRED_RECPRDS[min_eval_loss_epoch[1]]) + '\n' + str(spearmans_records[min_eval_loss_epoch[1]]) + '\n')
    print("\n\n")
    ...
    return MAE_RECORDS[min_eval_loss_epoch[1]], MDAE_RECORDS[min_eval_loss_epoch[1]], spearmans_records[min_eval_loss_epoch[1]]
...
def main():
    ...
    for file in TRAIN_TEST_FILE_PAIRS:
        ...
        mae, mdae, sp = train_eval_test(file_pair, train_dataloader, val_dataloader, all_test_dataloader, MODEL, test_file_names)
        res.append({"Data": file_pair, "MAE": mae, "MDAE": mdae, "Spearman": sp})
        del MODEL
        torch.cuda.empty_cache()            
        global OUTPUT
        with open('../Results/' + str(file_pair[:-4]) +'.txt', 'w+') as f:
            f.writelines(OUTPUT)
